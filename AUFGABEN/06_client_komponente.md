...
- Baue zwei Komponenten:
  - eine `RelatedArticleCard`-Komponente, die einen einzelnen "Related Artikel" darstellt (z.B. dessen Titel und Bild). Ohne Buttons etc.
    - Dafür bitte **nicht** die existierende `RelatedArticleBox`-Karte verwenden!
  - die Client-Komponente `RelatedArticleSlider`, die eine Liste von `RelatedArticle`-Objekten übergeben bekommt. Diese Komponente soll `RelatedArticleCard` verwendeen, um den jeweils ausgewählten "Related Artikel" darzustellen
...
- Server and client components (Next.js Doku): https://nextjs.org/docs/app/getting-started/server-and-client-components
